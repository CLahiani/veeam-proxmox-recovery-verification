...
|---|---|---|---|---|---|
| **CP00** | Authentification : REST VBR (OAuth), API Proxmox (jeton), SSH vers le nœud (`pveversion`) | Les trois réussissent ; le détail indique la version d'API et `pveversion` | — | L'une échoue | ✔ |
...
| **CP12** | La session de publication Data Integration API se termine en `Success` / `Warning` **et** de nouvelles images disque brutes apparaissent sous `MountRoot` | `Success`, disques listés | `Warning` | Session `Failed` / délai dépassé, ou aucune image → reste `SKIP` | `-WhatIf` |

Entre CP12 et l'étape 2, le script crée les overlays et la VM (`qemu-img`, `qm create`, `qm start`) ; une erreur à ce stade est rapportée en CP12 `KO` (`échec au lancement`).
...
| **CP23** | Ping depuis la sonde (`-PingCheck`) | Réponse | — | Aucune réponse | Désactivé / pas d'IP |
| **CP30** | Contrôles applicatifs, un CP par contrôle (`Applicatif : <Label>`) | Réussi | — | Échec (raison) | Pas d'IP / aucun contrôle / `SqlServer` absent / type inconnu |
